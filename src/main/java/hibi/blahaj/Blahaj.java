...
public class Blahaj implements ModInitializer {
	...
	public void onInitialize() {
		...
		try {
			...
			Blahaj.LOGGER.info("Polytrinkets is installed! Enabling support...");
		} catch (NoClassDefFoundError e) {
			Blahaj.LOGGER.warn("Polytrinkets is not installed! Registering items without Trinket support...");
	...
	private static void registerTrades() {
		TradeOfferHelper.registerVillagerOffers(VillagerProfession.SHEPHERD, 5, factories -> {
			factories.add((entity, random) -> new TradeOffer(new TradedItem(Items.EMERALD, 15),
					new ItemStack(BlahajBlocks.GRAY_SHARK_BLOCK), 2, 30, 0.1f));
		});
	}
